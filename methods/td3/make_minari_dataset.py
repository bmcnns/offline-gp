...
def evaluate(model_path, env_name, seed, num_timesteps):
    ...
    while t < num_timesteps:
        # this line is modified so that we can retain the seed so we can replicate the environment
        #state, terminated, truncated = eval_env.reset(seed=(seed+t)), False, False
        state, terminated, truncated = eval_env.reset(seed=seed), False, False
        state = state[0]
...
def save_dataset(env, env_name):
    ...
    dataset = env.create_dataset(
        # this line has been modified for a single episode capture
        #dataset_id=f"{envname_without_version}-Expert-{env_version_number}",
        dataset_id=f"Minimal-{envname_without_version}-Expert-{env_version_number}",
        algorithm_name="TD3",
